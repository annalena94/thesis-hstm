...
def format_as_latex(list_of_topics, topic_weights):
    n_topics = len(list_of_topics)
    latex_str = ''
    # round topic weights
    topic_weights = np.round(topic_weights, 3)
    for k in range(n_topics):
        latex_str += 'Topic ' + str(k) + '&' + ', '.join(list_of_topics[k]) + '&' + str(topic_weights[k])
        latex_str += '\\\\' + '\n'
    return latex_str
...
class Evaluator:
    ...
    def visualize_topics(self, num_words=7, format_pretty=False):
        ...
        topic_weights = self.topic_weights  # correct?
        for k in range(num_topics):
            ...
            if not format_pretty:
                ...
                print('Topic {} with {}: {}'.format(k, topic_weights[k], topic_words))
            else:
                topic_words = [self.vocab[t] for t in top_words]
                topics_list.append(topic_words)
                print('Topic {} with {}: {}'.format(k, topic_weights[k], ' '.join(topic_words)))
        print("---" * 60)
        latex_str = format_as_latex(topics_list, topic_weights)
        return latex_str
    ...
    def visualize_supervised_topics(self, num_words=10, normalize=True, pos_topics=True, format_pretty=False,
                                    compare_to_bow=False):
        num_topics = self.topics.shape[0]
        topics_list = []
    ...
    def show_text_features(self, index, n_words=10):
        ...
        print("Words in this document with biggest BOW weights:",
              [(valid_vocab[b], round(bow_scores[b], 3)) for b in top_bow])
        bow_tex = 'Words in doc w/ largest $\\omega$' + '&'.join(valid_vocab[top_bow])

        print("Words in this document with biggest gamma weights:",
              [(valid_vocab[b], round(gamma_scores[b], 3)) for b in top_gamma])
    ...
    def get_normalized_pmi_df(self, num_words=10, topics_to_use='neu'):
        ...
        # TODO: try calculating NPMI for negative and positive topic weights only
        # Idea: maybe they average out??
        for k in range(num_topics):
            npmi_total = 0
    ...
    def get_normalized_pmi(self, num_words=10):
        ...
        for k in range(num_topics):
            ...
            for (w1, w2) in it.combinations(top_words, 2):
                ...
                npmi_total += numerator / denom
                n += 1
            per_topic_npmi[k] = npmi_total / n  # / n
        return per_topic_npmi.mean()
